...
export const EquationDuels: React.FC<EquationDuelsProps> = ({
  challenge,
  onComplete,
  onAbandon
}) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.validation.isCorrect) {
        ...
        // Mark coefficients as correct
        setCoefficientSlots(prev => prev.map(slot => ({ ...slot, isCorrect: true })));
        
        setTimeout(() => onComplete(result), 2000);
      } else {
        // Incorrect answer - lose health and mana, trigger explosion
        const manaLoss = 15;
        const healthLoss = 25;
        ...
        // Mark coefficients as incorrect
